...
export function buildVolumeTable(meshes, volume, scene) {
  ...
  meshes.forEach((mesh, idx) => {
    ...
    const row = createVolumeRow(mesh, idx, computedVolume);
...
function createVolumeRow(mesh, idx, computedVolumes) {
...
function createVolumeValues(computedVolumes) {
  const container = document.createElement('div');
  ...
  mLText.innerText = computedVolumes.mL.toFixed(2) + ' cm3';
