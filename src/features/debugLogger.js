...
export async function logNrrdHeader(blob) {
  const text = await blob.text();
  const header = text.split("\n\n")[0];
  console.log("===== [NRRD Header 로그] =====");
  console.log(header);
  console.log("→ sizes:", (header.match(/sizes:\s+([\d\s]+)/) || [])[1]);
  console.log("→ origin:", (header.match(/space origin:\s+\((.*?)\)/) || [])[1]);
}
