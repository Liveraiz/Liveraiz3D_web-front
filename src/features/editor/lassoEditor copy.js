...
export class LassoEditor {
    constructor(canvas, camera, renderer, scene, controls, nvRender = null, nvMulti = null) {
        ...
        // ✅ 좌표계 보정 설정
        this.coordinateOffset = { x: 0, y: 0, z: 0 }; // 메시-볼륨 좌표 보정값
        this.useLPSToRASConversion = false; // LPS ↔ RAS 좌표계 변환 사용 여부
    ...
    validateMeshVolumeAlignment(mesh) {
        ...
        // 메시의 몇 개 정점을 샘플링하여 테스트
        const geometry = mesh.geometry;
        const positions = geometry.attributes.position.array;
        const vertexCount = positions.length / 3;
        // 더 많은 샘플로 정확한 정렬률 측정
        const sampleIndices = [];
        const sampleCount = Math.min(20, vertexCount); // 최대 20개 샘플
        ...
        console.log(`📊 정점 샘플링: 총 ${vertexCount}개 정점 중 ${sampleIndices.length}개 샘플 테스트`);
        ...
        for (const i of sampleIndices) {
            ...
            if (voxelPos) {
                ...
                if (idx >= 0 && idx < volume.img.length) {
                    ...
                    console.log(`정점 ${i}: 메시(${vertexPos.x.toFixed(1)}, ${vertexPos.y.toFixed(1)}, ${vertexPos.z.toFixed(1)}) → 복셀(${voxelPos.x}, ${voxelPos.y}, ${voxelPos.z}) = 값 ${voxelValue}`);
    ...
    applyBackendMeshTransformReverse(pos) {
        // 백엔드에서 적용한 변환을 역으로 적용:
        // 백엔드: verts_edited[:, 0] *= -1  (X축 반전)
        // 백엔드: verts_edited = verts_edited[:, [0, 2, 1]]  (X,Y,Z → X,Z,Y)

        // 역변환: (X,Z,Y) → (X,Y,Z) 그리고 X축 복원
    ...
    projectToNiivueRender(worldPos) {
        try {
            ...
            console.warn("⚠️ Niivue Render 좌표 변환 실패:", error);
            return null;
        }
    }
    ...
    applyVolumeEdit(lassoPoints) {
        if (!this.nvMulti || !this.nvMulti.volumes || !this.nvMulti.volumes[1]) {
            console.warn("⚠️ 편집할 볼륨이 없습니다.");
            return;
        }

        const segVolume = this.nvMulti.volumes[1];
        if (!segVolume.img || segVolume.img.length === 0) {
            console.warn("⚠️ 볼륨 데이터가 없습니다.");
            return;
        }

        try {
            console.log("🔧 볼륨 편집 시작...");
            console.log(`📍 라쏘 포인트 수: ${lassoPoints.length}, 캔버스: ${this.currentCanvas}`);
            ...
            if (editedVoxels > 0) {
                // ✅ Niivue 세그멘테이션 볼륨 강제 업데이트
                try {
                    this.nvMulti.updateGLVolume(); // 내부 GPU 텍스처 갱신
                } catch (error) {
                    console.warn("⚠️ updateGLVolume 실패 → refreshLayers 대체");
                ...
                console.log(`✅ 볼륨 편집 완료: ${editedVoxels}개 복셀 수정`);
            }

        } catch (error) {
            console.error("❌ 볼륨 편집 실패:", error);
        }
    }
    ...
    editSingleVolumeData(volume, viewerName) {
        ...
        console.log(`🎯 ${viewerName} 테스트 시작: ${editModeText} 복셀을 255로 변경`);
        console.log(`📦 ${viewerName} 볼륨 크기:`, volume.img.length);
        console.log(`📦 ${viewerName} 현재 볼륨 상태:`);
        ...
        console.log("  - LUT 길이:", volume.lut ? volume.lut.length : 'null');
                ...
                for (let y = 0; y < dims[1]; y++) {
                    for (let x = 0; x < dims[0]; x++) {
                        ...
                        if (screenPos) {
                            ...
                            if (isInside) {
                                volume.img[idx] = 255; // 편집된 부분으로 변경
                                changedVoxels++;
